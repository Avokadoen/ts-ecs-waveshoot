...
export class WaveShooterGame {
  ...
  readonly ENEMIES_SPEED = 210;
  readonly BULLET_SPEED = 500;
  readonly PLAYER_SPEED = 250;

  readonly ENEMY_ATTR = new Rectangle('#ff4081', { x: 20, y: 30 });
  readonly PLAYER_ATTR = new Rectangle('#3f51b5', { x: 20, y: 30 });
  readonly BULLET_ATTR = new Rectangle('#ffffff', { x: 4, y: 4 });

  playerScore: number;

  get difficultyScaling(): number {
    const ceiling = 2000;
  ...
  constructor(private canvas: HTMLCanvasElement) {
...

    {
      ...
      this.manager.registerSystem(drawRectangleSystem, query);
...

    {
      ...
      this.manager.registerSystem(movableSystem, query);
    }

    {
      ...
      this.manager.registerSystem(playerMoveSystem, query);
    }

    {
      ...
      const despawnBulletsSystem = (delaTime: number, args: Component<object>[], sharedArgs: Component<object>[]) => {
        ...
        if (Math.abs(magnitude(bMovable.position) - magnitude(pMovable.position)) > bullet.maxRange) {
          this.manager.removeComponent(args[0].entityId, Movable.identifier);
        }
      };

      this.manager.registerSystem(despawnBulletsSystem, query);
    }

    {
      ...
      const spawnEnemySystem = (_: Event, args: Component<any>[]) => {
        ...
        let ySpawn: number;
        if (xIsSmooth) {
          xSpawn = Math.random() * this.canvas.width;
          const yOffset = Math.random() * 200;
          ySpawn = (Math.random() > 0.5) ? this.canvas.height + yOffset : -yOffset;
        } else {
          ySpawn = Math.random() * this.canvas.height;
          const yOffset = Math.random() * 200;
          xSpawn = (Math.random() > 0.5) ? this.canvas.width + yOffset : -yOffset;
        }

        const enemyId = this.manager.queryEntities(enemyQuery).entities[0]?.id;

        if (!isNaN(enemyId)) {
          this.manager.addComponent(enemyId, new Movable(
            {
              x: xSpawn,
              y: ySpawn,
            },
      ...
      const spawnEnemyEvent = timer(2000, 1000).pipe(
        delay(Math.random() * (3 - (3.5 * this.difficultyScaling)))
      );
      ...
      const onSpawnEnemy = this.manager.registerEvent(spawnEnemySystem, enemySpawnQuery);
      spawnEnemyEvent.subscribe(() => this.manager.onEvent(onSpawnEnemy, null));
    }

    {
      ...
      const targetPlayerSystem = (delaTime: number, args: Component<object>[], sharedArgs: Component<object>[]) => {
        // const enemy = args[0].data as Enemy;
      ...
      this.manager.registerSystem(targetPlayerSystem, enemyQuery);
    }
      ...
      const bulletHitSystem = (_: number, args: Component<object>[], sharedArgs: Component<object>[]) => {
        ...
        for (let i = 0; i < sharedArgs.length; i += 3) {
          ...
          if (isColling(bRect, bMov, enemyRect, enemyMov)) {
            this.manager.removeComponent(sharedArgs[i + 1].entityId, Movable.identifier);
            this.manager.removeComponent(args[1].entityId, Movable.identifier);
            i = sharedArgs.length;
          }
        }
      };

      this.manager.registerSystem(bulletHitSystem, query);
    }

    {
      ...
      const userClickSystem = (event: Event, args: Component<any>[]) => {
        ...
        const clickDelta = {
          x: e.clientX - (movable.position.x + rectangle.size.x * 0.5) - this.canvas.offsetLeft,
          y: e.clientY - (movable.position.y + rectangle.size.y * 0.5) - this.canvas.offsetTop
        };
        ...
        const bulletId = this.manager.queryEntities(bulletQuery).entities[0].id;
        this.manager
          .addComponent(bulletId, new Movable(
          {
      ...
      const onUserClick = this.manager.registerEvent(userClickSystem, query);
      fromEvent(canvas, 'click').subscribe(e => this.manager.onEvent(onUserClick, e));
    }
